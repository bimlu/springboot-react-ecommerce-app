...
2. Set the environmental variables which will be impacted on docker-compose.yml.
    ...
    2. Rename the file "client/.env-example" to ".env".
    3. Stop the mysql.service and redis-server.service if they are running. using the command "sudo systemctl stop <SERVICE_NAME>".
    4. (Optional Step) You need to create a Stripe account and Google OAuth credentials.
